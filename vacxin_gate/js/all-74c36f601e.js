...
//silder pl responsive
jQuery(document).ready(function($) {
  jQuery("#a-newproduct-slider").owlCarousel({
...
jQuery(document).ready(function($) {
  jQuery("#a-sponser-slider").owlCarousel({ 
...
jQuery(document).ready(function($) {
  jQuery("#a-video-right-slider").owlCarousel({ 
...
jQuery(document).ready(function($) {
    typeof $.typeahead === 'function' && $.typeahead({
        ...
        template: function (query, item) {
            ...
            switch(item.type) {
                case 'medicines':
                    typeDisplay = '(Tủ thuốc)';
                    break;
                case 'guides':
                    typeDisplay = '(Cẩm nang)';
                    break;
                case 'faqs':
                    typeDisplay = '(Hỏi đáp)';
                    break;
            ...
            return '<span><span class="search-thumb-span">' +
                '<img src="{{thumb}}" class="search-result-thumb">' +
                "</span>" +
                '<span class="search-result-title">{{name}} <small>'+typeDisplay+'</small></span></span>'
        },
...
var owl = $('#a-banner-slider'); // save reference to variable
...
jQuery(document).ready(function($) {
  jQuery(".a-btn-search").click(function(){
    jQuery(".a-top-search-form-f").show();
...
jQuery(document).mouseup(function (e){
    ...
    {
        container.hide();
        jQuery(".a-btn-search").css('z-index',999);
    }
});

/**
 * Created by huongnx on 02/06/2016.
 */
var Global = {
    init: function () {
        this.config();
        ...
        this.registration();
        this.changePassword();
        this.quickMenuSlider();
    },
    
    quickMenuSlider: function () {
        var quickOwl = $('#quick-menu-slider');
        quickOwl.owlCarousel({
            items: 6,
            dots: true,
    ...
    showModalRegister: function () {
        $('#a-login-popup').modal('hide');
        this.openNewWindow(LOGIN_URI);
        $('html, body').addClass('modal-open');
    },

    loadIframeBox: function (url) {
        this.openNewWindow(url);
        $('html, body').addClass('modal-open');
    },

    openNewWindow: function (url) {
        var popUp = $('#oauthModal');
        popUp.modal();
        popUp.find('.modal-content').html("<iframe frameborder='0' src='" + url + "' width='100%' height='410'></iframe>");
    },
        ...
        $.ajaxSetup({
            ...
            statusCode: {
                401: function(xhr){
                    ...
                    noty({
                        type: 'warning',
                        text: 'Chưa đăng nhập. Vui lòng đăng nhập để tiếp tục',
                        callback: {
                            afterShow: function () {
                                $('.modal').modal('hide');
                                $('#a-login-popup').modal({
                ...
                500: function (xhr) {
                    noty({
                        type: 'warning',
                        text: 'Có lỗi xảy ra, bạn sẽ quay về trang chủ sau 5s.',
                        callback: {
                            afterShow: function () {
                                // location.reload();
                            }
    ...
    registration: function () {
        ...
        regForm.submit(function (e) {
            ...
            $.ajax({
                ...
                data:formData,
                beforeSend: function () {
                    regForm.find('fieldset').attr('disabled', 'disabled');
                },
                complete:function(response){
                    regForm.find('fieldset').removeAttr('disabled');
                    if(response.status == 302) {
                        $('#a-reg-popup').modal('hide');
                        noty({text: 'You have been logged in the System!!!'});
                        ...
                        if (data.status) {
                            ...
                            $('#a-xt-popup').html(data.confirm_box).modal({
                                show: true,
                                backdrop: 'static',
                                keyboard: false
                            });
                            ...
                            $.each(errors, function (key, value) {
                                var input = $('input[name=' + key + ']', regForm);
                                if (index == 0) {
                                    input.focus();
                                }
    ...
    verify: function () {
        ...
        verifyForm.submit(function (e) {
            ...
            $.ajax({
                ...
                data:formData,
                beforeSend: function () {
                    verifyForm.find('fieldset').attr('disabled', 'disabled');
                },
                complete:function(response){
                    verifyForm.find('fieldset').removeAttr('disabled');
                    if(response.status == 401) {
                        ...
                        if (data.status) {
                            ...
                            } else if(data.reset_password_box) {
                                $('#a-xt-popup').html(data.reset_password_box);
                            }
                        } else  {
                            var index = 0;
                            $.each(messages, function (key, value) {
                                var input = $('input[name=' + key + ']', verifyForm);
                                if (index == 0) {
                                    input.focus();
                                }
                                input.after('<span class="error">'+value+'</span>');
    ...
    login: function () {
        ...
        loginForm.submit(function (e) {
            ...
            $.ajax({
                ...
                data:formData,
                beforeSend: function () {
                    loginForm.find('fieldset').attr('disabled', 'disabled');
                },
                success:function(response){
                    loginForm.find('fieldset').removeAttr('disabled');
                    $('.error').remove();
                    ...
                    if (response.status != false) {
                        ...
                        $.each(response.messages, function (key, value) {
                            var input = $('input[name=' + key + ']', loginForm);
                            if (index == 0) {
                                input.focus();
                            }
                            input.after('<span class="error">'+value+'</span>');
    ...
    changePassword: function () {
        ...
        changePassForm.submit(function (e) {
            ...
            $.ajax({
                ...
                data:formData,
                beforeSend: function () {
                    changePassForm.find('fieldset').attr('disabled', 'disabled');
                },
                success:function(response){
                    changePassForm.find('fieldset').removeAttr('disabled');
                    $('.error').remove();
                    ...
                    if (response.status != false) {
                        ...
                        var index = 0;
                        $.each(response.messages, function (key, value) {
                            var input = $('input[name=' + key + ']', changePassForm);
                            if (index == 0) {
                                input.focus();
                            }
                            input.after('<span class="error">'+value+'</span>');
    ...
    resetPassword: function (obj) {
        $.ajax({
            ...
            beforeSend: function () {
                obj.parents('form').find('fieldset').attr('disabled', 'disabled');
            },
            success: function (response) {
                obj.parents('form').find('fieldset').removeAttr('disabled')
                if (response.status != false) {
                    noty({
                        type: 'success',
                        text: response.messages,
                    ...
                    if (response.messages) {
                        ...
                        $('.error').remove();
                        $.each(response.messages, function (key, value) {
                            var input = $('input[name=' + key + ']', obj.parents('form'));
                            if (index == 0) {
                                input.focus();
                            }
                            input.after('<span class="error">'+value+'</span>');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status != false) {
                    ...
                    $('#a-login-popup').modal('hide');
                    $('#a-xt-popup').html(response.confirm_box).modal({
                        show: true,
                        backdrop: 'static',
                        keyboard: false
                    });
    ...
    loadPackageBox: function () {
        var packageBox = $('#a-package-popup');

        $.ajax({
            url: packageBox.data('url'),
            type: 'GET',
            dataType: 'HTML',
            beforeSend: function () {
                packageBox.modal({
            ...
            success: function (data) {
                packageBox.find('.package-register').html(data);
            }
        });
    },
    ...
    confirmUnRegisterPackage: function (obj) {
        obj.addClass('disabled');
        noty({
            text: 'Bạn có chắc chắn muốn hủy gói gói dịch vụ Thông tin Vắc xin?',
            layout: 'topCenter',
            buttons: [
                {addClass: 'btn btn-primary', text: 'Ok', onClick: function($noty) {
                    $noty.close();
                    Global.registerPackage(obj);
                }
                },
