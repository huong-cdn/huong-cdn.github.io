...
/* PLEASE DO NOT COPY AND PASTE THIS CODE. */(function() {if (!window['___grecaptcha_cfg']) { window['___grecaptcha_cfg'] = {}; };if (!window['___grecaptcha_cfg']['render']) { window['___grecaptcha_cfg']['render'] = 'onload'; };window['__google_recaptcha_client'] = true;var po = document.createElement('script'); po.type = 'text/javascript'; po.async = true;po.src = 'https://www.gstatic.com/recaptcha/api2/r20160307141848/recaptcha__vi.js';var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(po, s);})();
...
/**
 * Created by huongnx on 04/03/2016.
 */
/*!
...
/**
 * Created by huongnx on 23/01/2016.
 */
...
//        return false;
    ...
    return true;
}, 'Sai định dạng ngày (Nhập số có 2 chữ số từ 01 đến 31)');
...
//        return false;
    ...
    return true;
}, 'Sai định dạng tháng (Nhập số có 2 chữ số từ 01 đến 12)');

$.validator.addMethod("checkYear", function (value) {
    var currVal = value;
    ...
    if ((currVal < 2014) || (currVal > current_year)) {
        return false;
    }
    return true;
}, 'Năm sai định dạng (Chỉ được nhập số có 4 chữ số và 2014 <= năm <=' + new Date().getFullYear() + ')');

$.validator.addMethod("checkName", function (value) {
    var currVal = value.trim().length;
    return currVal > 0;
...
$.validator.addMethod("isDate", function (value, element, params) {
    var selector = params[0],
        classSelector = params[1],
        daySelector = selector[0],
        monthSelector = selector[1],
        yearSelector = selector[2],
        dtDay = parseInt($(daySelector).val(), 10),
        dtMonth = parseInt($(monthSelector).val(), 10);
    ...
    if (year && month && day) {
        ...
        var valid = d && (d.getMonth() + 1) == month && d.getDate() == Number(day);
    ...
    return true;
}, 'Ngày bạn nhập không tồn tại');

$.validator.addMethod("isIdNumber", function (value) {
    return value.match(/^([a-zA-Z]\d{7})$/) ||
            value.match(/^([0-9]\d{8})$/) ||
            value.match(/^([0-9]\d{11})$/) ||
            value.match(/^([0-9]\d{7})$/);
...
var Global = {
    ...
    beforeRegister: function() {
        var d = new Date(),
            ...
            dateLine = new Date(REGISTER_TIME_OPEN),
            timeCountDown = dateLine.getTime() - currentTime;
        ...
        if (REGISTER_OPEN == 0) {
            ...
            setTimeout(function(){
                REGISTER_OPEN = 1;
                $('#form-check-name').removeClass('disabled');
            }, timeCountDown);
        }
    },
    reloadPage: function() {
        $(window).bind('beforeunload', function(){
            return 'Bạn có thực sự muốn tải lại trang không. Tất cả các thông tin bạn đã nhập sẽ không được lưu trên hệ thống';
        });
    },
    localStorage: function() {
        if (window.location.hash.slice(1) == 'success') {
            localforage.getItem('formSuccess', function(err, value) {
                if (value) {
    ...
    disabledDiv: function () {
        $('#form-check-date').find('input').prop('disabled', true);
    },
    enableDiv: function () {
        $('#form-check-date').find('input').prop("disabled", false);
    },
    requestAjax: function () {
        ...
        var dateFields = ["#VaccineDay", "#VaccineMonth", "#VaccineYear"];
        ...
        form.validate({
            ...
            rules: {
                ...
                id_number: {
                    required: true,
                    //number: true,
                    isIdNumber: true // nhap chu va so tu 8 den 12 ky tu
                },
                mobile: {
                    required: true,
                    number: true,
            ...
            messages: {
                ...
                child_name: {
                    required: "Bạn chưa nhập họ và tên bé"
                },
                id_number: {
                    required: "Bạn chưa nhập số CMND/Hộ chiếu"
                },
                mobile: {
    ...
    confirmRegister: function () {
        $('.stepNext').click(function (e) {
            e.preventDefault();
            $('#form-check-name').addClass('disabled');
            $('#form-check-date').removeClass('disabled');
            Global.enableDiv();
            Global.validateStepTwo();

            var dtDay = $('#VaccineDay').val(),
                dtMonth = $('#VaccineMonth').val(),
                dtYear = $('#VaccineYear').val(),
                dateStr = dtYear + '/' + dtMonth + '/' + dtDay;

            $('.item-vaccine').each(function () {
                var personAge = monthDiff(new Date(dateStr), new Date($(this).data('date')));

                if (personAge > 24 || personAge < 2) { // Chi cho dang ky neu tuoi cua ben toi ngay tiem trong khoang  2 - 24 thnag tuoi
        ...
        function monthDiff(d1, d2) {
            var dayInMonth1 = Math.round(((new Date(d1.getFullYear(), d1.getMonth())) - (new Date(d1.getFullYear(), d1.getMonth() - 1))) / 86400000)
            var dayInMonth2 = Math.round(((new Date(d2.getFullYear(), d2.getMonth())) - (new Date(d2.getFullYear(), d2.getMonth() - 1))) / 86400000)

            var dayInMonth = dayInMonth1;
            if (dayInMonth < dayInMonth2) {
                dayInMonth = dayInMonth2;
            }
            return (d2.getDate() - d1.getDate()) / dayInMonth +
    ...
    validateStepTwo: function () {
        $('.inputCheck').each(function () {
            if ($(this).is(":checked") && $('.check_dk').is(":checked") && $('#recaptcha_response_field').val() != '') {
                $('#form-check-date').find('.btn_reg').removeClass('disabled');
                return false;
            } else {
                $('#form-check-date').find('.btn_reg').addClass('disabled');
            }
        });

        $('.check_dk').click(function () {
            var that = $(this);
            $('.inputCheck').each(function () {
                if ($(this).is(":checked") && that.is(":checked") && $('#recaptcha_response_field').val() != '') {
                    $('#form-check-date').find('.btn_reg').removeClass('disabled');
                    return false;
                } else {
                    $('#form-check-date').find('.btn_reg').addClass('disabled');
        ...
        $('.inputCheck').click(function () {
            ...
            if ($box.is(":checked")) {
                ...
                $(group).parents('.checkbox').removeClass('box_checked');
                $box.prop("checked", true);
            } else {
                $box.prop("checked", false).removeClass('box_checked');
            }

            $box.parents('.checkbox').toggleClass("box_checked");
        ...
        Global.submitResgiter();
    },
    submitResgiter: function () {
        var form = $('#register');
        $('.btnRegister').click(function () {
            if (REGISTER_OPEN != 0) {
                ...
                $('#fail').find('.registerBack').html('Đóng');
                $('#fail').find('.registerBack').removeClass('registerBack').attr('data-dismiss', 'modal');
                Global.showError({mess: "Thời gian đăng ký phiếu tiêm vắc xin Pentaxim 5 trong 1 bắt đầu từ 09:00 giờ sáng ngày 08/03/2016"});
            }
    ...
    showError: function (errors) {
        ...
        $('#process').modal('hide');
        $('#fail .modal-body').html(modalBody);
        $('#fail').modal({
            backdrop: 'static',
            keyboard: false
        });
        ...
        $('.registerBack').click(function () {
            ...
            // disable thông báo reload của trình duyệt
            $(window).unbind('beforeunload');
            location.reload(true);
        });
    },
    checkRegisterStatus: function (id) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    switch (response.status) {
                        case REGISTER_STATUS_APPROVED:
                            clearInterval(interval);
                            window.location = response.url;
                            break;
                        case REGISTER_STATUS_REJECT:
                            clearInterval(interval);
                            Global.showError(response.errors);
                            break;
                    }
    ...
    validateBirthDay: function () {
        var dtDay = $('#VaccineDay').val().length >= 2 ? $('#VaccineDay').val() : '0' + $('#VaccineDay').val(),
                dtMonth = $('#VaccineMonth').val().length >= 2 ? $('#VaccineMonth').val() : '0' + $('#VaccineMonth').val(),
                dtYear = $('#VaccineYear').val(),
                dateStr = dtYear + '/' + dtMonth + '/' + dtDay,
        ...
        if (REGISTER_OPEN != 0) {
            if (parseDate(BIRTH_START) > date || date > parseDate(BIRTH_END)) {
                Global.showError({mess: "Rất tiếc, tuổi của trẻ không hợp lệ, vắc xin này chỉ dành cho trẻ từ 2-24 tháng tuổi"});
                return false;
            } else {
                $('#confirm').modal({
                    backdrop: 'static',
            ...
            Global.showError({mess: 'Dữ liệu của bạn được nhập trước giờ đăng ký. Vui lòng nhập lại!'});
        }
    },
};
